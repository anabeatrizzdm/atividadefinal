...
void cabecalho();
void aloc_dina(); //ALOCACAO DINAMICA DE MEMORIA (DEMO)
...
void lista_ordem_AZ(struct Produto produtos[], int quant_produt, struct Fabricante fabricantes[], int quant_fab);
void lista_ordem_ZA(struct Produto produtos[], int quant_produt, struct Fabricante fabricantes[], int quant_fab);
...
int main() {
  ...
  system("chcp 1252 && color 0f");
  
  //ALOCACAO DINAMICA DE MEMORIA
  aloc_dina();
  
  //CADASTRO
  quant_fab = cadastro_fab(fabricantes);
  quant_prod = cadastro_prod(fabricantes, produtos, quant_fab);
  ...
  while(1){
    menu();
    do{
      printf("escolha um numero valido(0 a 11):\n");
    scanf("%d",&escolhamenu);
    }while(escolhamenu<0||escolhamenu>11);
    getchar();
    printf("\e[1;1H\e[2J");
    ...
    if(escolhamenu==10){
      ...
      lista_ordem_AZ(produtos,quant_prod,fabricantes,quant_fab);
      printf("\n[0] Voltar ao menu\n[9] Encerrar programa\n");
      scanf("%d",&escolhamenu);
      ...
      lista_ordem_ZA(produtos,quant_prod,fabricantes,quant_fab);
      printf("\n[0] Voltar ao menu\n[9] Encerrar programa\n");
      scanf("%d",&escolhamenu);
...
void aloc_dina() {
  char *mensagem = (char*)malloc(300 * sizeof(char)); //ponteiro recebe 1o endereco dos espacos alocados
  strcpy(mensagem, "    Ana\n    Christian\n    Efraim\n    Matheus\n    Talys\n\n    Diego\n\n        UCB, 2023");
  cabecalho();
  printf("%s", mensagem);
  system("timeout 30 >nul");
  free(mensagem); //desalocacao (liberacao) dos espacos
}
...
void val_peso(struct Produto produtos[], int quant_prod, int i) {
  do {
    ...
    if (produtos[i].peso <= 0.05 || produtos[i].peso > 50) {
      printf("\n\t\tPESO INVALIDO! Ele deve estar entre 0.05 kg (50g) e 50 kg.");
      getch();
    }
  } while (produtos[i].peso <= 0.005 || produtos[i].peso > 50);
}
...
void val_valor_compra (struct Produto produtos[], int quant_prod, int i) {
  do {
    ...
    if (produtos[i].valorCompra < 0.5 || produtos[i].valorCompra > 8000) {
      printf("\n\t\tVALOR DE COMPRA INVALIDO! Ele deve estar entre R$ 0.50 e R$ 8000.00");
      getch();
    }
  } while (produtos[i].valorCompra < 0.5 || produtos[i].valorCompra > 8000);
}
...
void val_valor_venda (struct Produto produtos[], int quant_prod, int i) {
  do {
    ...
    if (produtos[i].valorVenda < 1 || produtos[i].valorVenda > 10000) {
      printf("\n\t\tVALOR DE VENDA INVALIDO! Ele deve estar entre R$ 1.00 e R$ 10000.00");
      getch();
    }
...
void menu(){
  ...
  printf("|[10] Listar todos produtos em ordem alfabetica                               |\n");
  printf("|[11] Listar todas as marcas em ordem alfabetica decrescente                  |\n");
  printf("|[0] Sair                                                                     |\n");
  printf("------------------------------------------------------------------------------\n");
}
...
void lista_ordem_AZ(struct Produto produtos[], int quant_produt, struct Fabricante fabricantes[], int quant_fab) {
    int ind_ord_cresc[100], i, j, indice_anterior, retorno;
    char descricao[50][100], descricao_anterior[100];

    for (i = 0; i < quant_produt; i++) {
        ind_ord_cresc[i] = i;
        strcpy(descricao[i], produtos[i].descricao);
    }

    for (i = 0; i < quant_produt - 1; i++) {
        for (j = 0; j < quant_produt - i - 1; j++) {
            retorno = strcmp(descricao[j], descricao[j + 1]);
            if (retorno > 0) {
                indice_anterior = ind_ord_cresc[j + 1];
                ind_ord_cresc[j + 1] = ind_ord_cresc[j];
                ind_ord_cresc[j] = indice_anterior;

                strcpy(descricao_anterior, descricao[j + 1]);
                strcpy(descricao[j + 1], descricao[j]);
                strcpy(descricao[j], descricao_anterior);
            }
        }
    }
...
void lista_ordem_ZA(struct Produto produtos[], int quant_produt, struct Fabricante fabricantes[], int quant_fab) {
    int ind_ord_cresc[100], i, j, indice_anterior, retorno;
    char marca[50][100], marca_anterior[100];

    for (i = 0; i < quant_fab; i++) {
        ind_ord_cresc[i] = i;
        strcpy(marca[i], fabricantes[i].marca);
    }

    for (i = 0; i < quant_fab - 1; i++) {
        for (j = 0; j < quant_fab - i - 1; j++) {
            retorno = strcmp(marca[j], marca[j + 1]);
            if (retorno > 0) {
                indice_anterior = ind_ord_cresc[j + 1];
                ind_ord_cresc[j + 1] = ind_ord_cresc[j];
                ind_ord_cresc[j] = indice_anterior;

                strcpy(marca_anterior, marca[j + 1]);
                strcpy(marca[j + 1], marca[j]);
                strcpy(marca[j], marca_anterior);
            }
        }
    }
    ...
    for (j = quant_fab - 1; j >=0 ; j--) {
      i=ind_ord_cresc[j];
        linha();
            printf(" MARCA: %s    ", fabricantes[i].marca);
            printf(" Site: %s    ", fabricantes[i].site);
            printf(" Telefone: %s    ", fabricantes[i].telefone);  
            printf(" UF (sigla): %s\n",fabricantes[i].uf);
            linha();
